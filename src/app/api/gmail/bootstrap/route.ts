...
const VALID_RANGES = [7, 14, 30, 60, 90] as const;
...
export async function POST(request: Request) {
  ...
  if (!VALID_RANGES.includes(days)) {
    return NextResponse.json({ ok: false, error: "days debe ser 7, 14, 30, 60 o 90" }, { status: 400 });
  }

  const supabase = createSupabaseServerClient();
  try {
    const connection = await getConnectionOrThrow(supabase);
    const { summary, preview } = await runBootstrapSync(supabase, connection, days as 7 | 14 | 30 | 60 | 90);
